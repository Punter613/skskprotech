...
echo "=========================================="
echo "🛡️ Deploying Hardened Bracket-Depth Diagnose Route 🛡️"
echo "=========================================="

# 1. Overwrite with your superior version
...
function extractJSON(text) {
  if (!text) return null;
  text = text.replace(/```json\s*/gi, '').replace(/
```\s*/g, '');
...
Tech Notes: ${notes.join(', ') || 'N/A'}`;
    ...
    // Merge with safeResult to guarantee all keys exist
    const result = { ...safeResult(), ...parsed };

    // Sanitize urgency — if AI went rogue
...
echo "✅ Senior patch syntax check passed flawlessly!"

# 3. Deploy it live
git add src/routes/diagnose.js
git commit -m "Implement advanced character bracket-depth JSON extractor and fix hardcoded vehicle telemetry trap in diagnose endpoint"
git push

echo "=========================================="
echo "🚀 SUCCESS! Your bulletproof engine upgrade is live on Render! 🚀"
echo "=========================================="

rm ship_hardened_diagnose.js
