...
echo "=========================================="
echo "🛡️ Shipping Hardened Bracket-Depth File (Line-Wrap Fixed) 🛡️"
echo "=========================================="
...
function extractJSON(text) {
  if (!text) return null;
  // Kept on safe single lines to bypass terminal paste bugs
  text = text.replace(/
```json\s*/gi, '');
...
echo "✅ Custom script syntax validation passed perfectly!"

# Push live
git add src/routes/diagnose.js
git commit -m "Deploy senior bracket-depth parser update with single-line terminal string safe formatting"
git push

echo "=========================================="
echo "🚀 SUCCESS! Your bulletproof scanner upgrade is live on Render! 🚀"
echo "=========================================="

rm patch_diagnose_final.sh
